...
import { deleteRecipe } from "@/server/mutations";
...
export default function DeleteRecipeModal({ recipeID }: RecipeOptionProps) {
  const [state, formAction, isPending] = useActionState(deleteRecipe, {
    errors: {},
  });
      ...
      <Modal className="bg-size-[200px_200px] w-[400px] flex-col rounded-lg bg-background bg-[url(/noisy-texture-200x200.png)] bg-repeat p-4">
        <Dialog>
          <Form action={formAction} className="flex h-full flex-col justify-between">
            ...
            <input className="hidden" name="recipeID" defaultValue={recipeID} />
            <div>
              <Heading className="font-ptSerif text-[32px] text-title">Delete the recipe?</Heading>
